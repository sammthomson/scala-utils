...
class FibonacciHeapTest extends FlatSpec with Matchers {
  "A FibonacciHeap" should "decrease priority correctly" in {
    val heap = new FibonacciHeap[Int, Int]()
    heap should be (empty)
    heap should have size 0
    val entries = m.Map.empty[Int, FibonacciHeap.Entry[Int, Int]]
    for (i <- 100 until 200) {
      entries += i -> heap.add(i, i)
    }
    heap should not be empty
    heap should have size 100
    heap.decreasePriority(entries(110), 50)
    heap.decreasePriority(entries(140), 25)
    heap.decreasePriority(entries(160), 15)
    heap.peekOption.get should equal (entries(160))
    heap.pollOption.get should equal (160)
    heap.peekOption.get should equal (entries(140))
    heap.remove(entries(140))
    heap.remove(entries(110))
    heap.peekOption.get should equal (entries(100))
    heap.clear()
    heap should be (empty)
    heap should have size 0
  }

  it should "handle duplicates correctly" in {
    val heap = new FibonacciHeap[Int, Double]()
    heap should be (empty)
    heap should have size 0
    val priority = Double.MinPositiveValue
    for (i <- 1 to 1000) {
      heap.add(i, priority)
    }
    heap should not be empty
    ...
    heap.pollOption
    heap should not be empty
    heap should have size 999
    heap.clear()
    heap should be (empty)
    heap should have size 0
  }

  it should "handle duplicates and one slightly larger" in {
    val heap = new FibonacciHeap[Int, Double]()
    heap should be (empty)
    heap should have size 0
    val priority = 0.0
    ...
    heap.add(1001, Double.MinPositiveValue)
    heap should not be empty
    heap should have size 1000
    heap.pollOption.get should be < 1001
    heap should not be empty
    heap should have size 999
    heap.clear()
    heap should be (empty)
  ...
  it should "handle duplicates and one slightly smaller" in {
    val heap = new FibonacciHeap[Int, Double]()
    heap should be (empty)
    heap should have size 0
    val priority = Double.MinPositiveValue
    ...
    heap should not be empty
    heap should have size 1000
    heap.pollOption.get should equal (1001)
    heap should not be empty
    heap should have size 999
    heap.clear()
    heap should be (empty)
  ...
  it should "merge correctly" in {
    val heap1 = new FibonacciHeap[Int, Int]()
    heap1 should be (empty)
    heap1 should have size 0
    for (i <- 1 to 9 by 2) {
      heap1.add(i, i)
    }
    heap1 should not be empty
    heap1 should have size 5
    val heap2 = new FibonacciHeap[Int, Int]()
    heap2 should be (empty)
    heap2 should have size 0
    for (i <- 2 to 10 by 2) {
      heap2.add(i, i)
    }
    heap2 should not be empty
    heap2 should have size 5
    val joined = FibonacciHeap.merge(heap1, heap2)
    joined should not be empty
    joined should have size 10
    var oVal = joined.pollOption
    var i = 1
    oVal.get should be (i)
    while (!joined.isEmpty) {
      oVal = joined.pollOption
      oVal.get should be > i
      i = oVal.get
    }
    joined should be (empty)
    joined should have size 0
  }

  it should "correctly sort a large array of Doubles" in {
    val values = Array(
    ...
    val heap = new FibonacciHeap[Double, Double]()
    for (d <- values) {
      heap.add(d, d)
    }
    ...
    var i = 0
    while (!heap.isEmpty) {
      heap.pollOption.get should equal (sorted(i))
      i += 1
    }
  }
}
